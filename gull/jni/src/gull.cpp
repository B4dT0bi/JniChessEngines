...
alignas(64) GPawnEntry PawnHash[pawn_hash_size];
#else
#define pawn_hash_size (32 * 1024)
alignas(64) GPawnEntry PawnHashOne[pawn_hash_size];
alignas(64) GPawnEntry PawnHashTwo[pawn_hash_size];
...
int TimeLimit1, TimeLimit2, HardwarePopCnt;
...
enum
{
	MatRB, MatRN, MatQRR, MatQRB, MatQRN, MatQ3, MatBBR, MatBNR, MatNNR, MatM
};
...
enum
{
	StormBlockedMul, StormShelterAttMul, StormConnectedMul, StormOpenMul, StormFreeMul
};
...
enum
{
	PasserOnePiece, PasserOpKingControl, PasserOpMinorControl, PasserOpRookBlock
};
...
enum
{
	IsolatedOpen, IsolatedClosed, IsolatedBlocked, IsolatedDoubledOpen, IsolatedDoubledClosed
};
...
enum
{
	UpBlocked, PasserTarget, ChainRoot
};
...
enum
{
	RookHof, RookHofWeakPAtt, RookOf, RookOfOpen, RookOfMinorFixed, RookOfMinorHaging, RookOfKingAtt, Rook7th, Rook7thK8th, Rook7thDoubled
};
...
enum
{
	TacticalMajorPawn, TacticalMinorPawn, TacticalMajorMinor, TacticalMinorMinor, TacticalThreat, TacticalDoubleThreat
};
...
enum
{
	KingDefKnight, KingDefBishop, KingDefRook, KingDefQueen
};
...
enum
{
	PawnChainLinear, PawnChain, PawnBlocked, PawnFileSpan
};
...
enum
{
	BishopNonForwardPawn, BishopPawnBlock
};
...
enum
{
	KnightOutpost, KnightOutpostProtected, KnightOutpostPawnAtt, KnightOutpostBishopAtt, KnightOutpostKingAtt
};
...
enum
{
	WeakPin, StrongPin, ThreatPin, SelfPawnPin, SelfPiecePin
};
...
forceinline int MaxF(int x, int y);
forceinline double MinF(double x, double y);
forceinline double MaxF(double x, double y);
...
void init_variables()
{
	...
	for (i = 0; i < curr_ind; i++) if (!indexed[i])
		{
...
start:
				...
				fprintf(f, "    ");
				int cnt = 0, index = 0;
				for (int k = 0; k < VarIndex[j]; k++) if (Active[k]) index++;
				char * p = SourceFile[i].line, *end;
				while ((p = strchr(p + 1, ',')) != NULL) cnt++;
				if (end = strstr(SourceFile[i + 1].line, "};")) cnt++;
				for (int k = 0; k < cnt; k++)
				{
...
{
	return -(log((1.0 / MinF(0.99999, Max(ratio, 0.00001))) - 1.0) / log(10.0)) * 400.0;
}

double rand_u()
{
	return MinF(1.0, Max(0.0, ((double) ((rand() << 15) | rand())) / (32768.0 * 32768.0)));
}
...
double play_position(double * one, double * two, int depth, char * fen, GMatchInfo * MI)
{
	...
	if (result >= 1.98) MI->wins++;
	else if (result <= 0.02) MI->losses++;
	else MI->draws++;
	score += result;
	result = play_game(two, one, depth, fen);
	if (result >= 1.98) MI->losses++;
	else if (result <= 0.02) MI->wins++;
	else MI->draws++;
	score += 2.0 - result;
	return score;
}
...
double match(double * one, double * two, int positions, int depth, GMatchInfo * MI)
{
	...
	{
#ifndef PGN
		score += play_position(one, two, depth, Fen[random64() % (uint64_t) opening_positions], MI);
#else
		score += play_position(one, two, depth, (char*) &Pos[random64() % (uint64_t) pgn_positions], MI);
#endif
	}
...
{
	uint64_t att = 0;
	for (uint64_t u = BMask[i]; T(u); Cut(u)) if (F(Between[i][lsb(u)] & occ)) att |= Between[i][lsb(u)] | Bit(lsb(u));
	return att;
}
...
{
	uint64_t att = 0;
	for (uint64_t u = RMask[i]; T(u); Cut(u)) if (F(Between[i][lsb(u)] & occ)) att |= Between[i][lsb(u)] | Bit(lsb(u));
	return att;
}
...
uint64_t random64()
{
	uint64_t key = Convert(rand16(), uint64_t);
	key <<= 16;
	key |= Convert(rand16(), uint64_t);
	key <<= 16;
	key |= Convert(rand16(), uint64_t);
	key <<= 16;
...
void init_misc()
{
	...
	TurnKey = random64();
	for (i = 0; i < 8; i++) EPKey[i] = random64();
	for (i = 0; i < 16; i++) CastleKey[i] = random64();
	for (i = 0; i < 16; i++) for (j = 0; j < 64; j++)
		{
			if (i == 0) PieceKey[i][j] = 0;
			else PieceKey[i][j] = random64();
		}
...
#endif
	char name[256];
	int64_t size = (hash_size * sizeof (GEntry));
...
forceinline GPVEntry * probe_pv_hash()
{
...
template <bool me> void do_move(int move)
{
	...
	if (F(Next->material & FlagUnusualMaterial)) prefetch((char *) (Material + Next->material));
...
template <bool me, bool HPopCnt> forceinline void eval_pawns(GPawnEntry * PawnEntry, GPawnEvalInfo &PEI)
{
	...
	int shelter = 0;
	uint64_t mpawns = Pawn(me) & Forward[me][me ? Min(kr + 1, 7) : Max(kr - 1, 0)];
	for (int file = start, i = 0; i < 3; file += inc, i++)
	{
		...
		if (Pawn(opp) & File[file])
		{
			...
			if ((rank = CRank(opp, sq)) < 6)
			{
				...
				if (uint64_t u = (PIsolated[File(sq)] & Forward[opp][Rank(sq)] & Pawn(me)))
				{
					int square = NB(opp, u);
	...
	for (uint64_t u = Pawn(me); T(u); u ^= b)
	{
		...
		if (up && next == IPawn(opp))
		{
			...
			{
				if (rrank <= 2)
				{ // IDEA (based on weird passer target tuning result): may be score unprotected/backward depending on rank/file?
...
template <bool me, bool HPopCnt> forceinline void eval_queens(GEvalInfo &EI)
{
	uint64_t u, b;
...
template <bool me, bool HPopCnt> forceinline void eval_rooks(GEvalInfo &EI)
{
	uint64_t u, b;
...
template <bool me, bool HPopCnt> forceinline void eval_bishops(GEvalInfo &EI)
{
	uint64_t u, b;
	for (u = Bishop(me); T(u); u ^= b)
	{
		...
		if (b & LightArea)
		{
			for (uint64_t v = ((~BishopForward[me][sq]) | (att & Forward[me][Rank(sq)])) & Pawn(opp) & (~Current->patt[opp]) & LightArea; v; Cut(v))
			{
				uint64_t square = lsb(v);
		...
		{
			for (uint64_t v = ((~BishopForward[me][sq]) | (att & Forward[me][Rank(sq)])) & Pawn(opp) & (~Current->patt[opp]) & DarkArea; v; Cut(v))
			{
				uint64_t square = lsb(v);
				if (!((PSupport[opp][square] | PWay[opp][square]) & Pawn(opp))) IncV(EI.score, Ca(BishopSpecial, BishopNonForwardPawn));
			}
...
template <bool me, bool HPopCnt> forceinline void eval_knights(GEvalInfo &EI)
{
	uint64_t u, b;
...
template <bool me, bool HPopCnt> forceinline void eval_king(GEvalInfo &EI)
{
...
template <bool me, bool HPopCnt> forceinline void eval_passer(GEvalInfo &EI)
{
	for (uint64_t u = EI.PawnEntry->passer[me]; T(u); Cut(u))
	{
		...
		if (!(way & Piece(opp)))
		{
			...
			for (uint64_t v = PAtt[me][sq - Push(me)] & Pawn(me); T(v); Cut(v))
			{
				int square = lsb(v);
...
template <bool me, bool HPopCnt> forceinline void eval_pieces(GEvalInfo &EI)
{
...
template <bool pv> forceinline int extension(int move, int depth)
{
	register int ext = 0;
	if (pv)
	{
...
template <bool me> forceinline void capture_margin(int alpha, int &score)
{
...
skip_iid:
	...
	if (hash_move && is_legal<me>(move = hash_move))
	{
		...
		{
#ifndef TUNER
			std::memset(Data + 1, 0, 127 * sizeof (GData));
#endif
...
#endif
	ci_h = GetStdHandle(STD_INPUT_HANDLE);
	ci_pipe = !GetConsoleMode(ci_h, &ci_dw);
	if (!ci_pipe)
	{
		SetConsoleMode(ci_h, ci_dw & ~(ENABLE_MOUSE_INPUT | ENABLE_WINDOW_INPUT));
		FlushConsoleInputBuffer(ci_h);
	}
	if (ci_pipe)
	{
		if (!PeekNamedPipe(ci_h, NULL, 0, NULL, &ci_dw, NULL))return true;
		return ci_dw;
	}
	else
	{
		GetNumberOfConsoleInputEvents(ci_h, &ci_dw);
		return ci_dw <= 1 ? 0 : ci_dw;
	}
    return false;
#endif
}

void epd_test(const std::string& filename, int time_limit)
{
	int n = 0, positions = 4000;
	uint64_t Time, all_nodes = 0, new_time, total_time;
	double prod = 0.0;
	char * ptr;
...
void uci()
{
	...
	const char* ret = fgets(mstring, 65536, stdin);
	if (!ret || feof(stdin)) exit(0);
	ptr = strchr(mstring, '\n');
	if (ptr != NULL) *ptr = 0;
...
int main(int argc, char *argv[])
{
	...
	if (parent)
	{
		if (GetProcAddress(GetModuleHandle(TEXT("kernel32")), "GetLogicalProcessorInformation") != NULL)
		{
...
reset_jump:
	...
	uint64_t ctime;
	QueryProcessCycleTime(GetCurrentProcess(), &ctime);
	srand(time(NULL) + 123 * GetProcessId(GetCurrentProcess()) + ctime);
	QueryProcessCycleTime(GetCurrentProcess(), &ctime);
	seed = (uint64_t) (time(NULL) + 345 * GetProcessId(GetCurrentProcess()) + ctime);
	init_openings();
	init_variables();
	...
	double New[1024];
	read_list("(5.07,27.02,27.37,15.16,28.60,14.62,40.93,8.61,14.02,172.58,178.09,180.83,457.03,128.24,172.66,178.21,343.44,1281.53,45.85)", New, active_vars);
	for (i = 7; i < 64; i++)
	{
		fprintf(stdout, "\ndepth = %d/%d: \n", i, i + 1);
