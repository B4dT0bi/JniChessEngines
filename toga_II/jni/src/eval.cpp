...
static /* const */ int PieceActivityWeight = 164; // 100%
static /* const */ int KingSafetyWeight = 246; // 100%
static /* const */ int PassedPawnWeight = 233; // 100%

static const int KnightUnit = 7;
static const int BishopUnit = 9;
static const int RookUnit = 11;
static const int QueenUnit = 20;

static const int MobMove = 1;
static const int MobAttack = 1;
static const int MobDefense = 0;

static const int KnightMobOpening = 3;
static const int KnightMobEndgame = 5;
static const int BishopMobOpening = 5;
static const int BishopMobEndgame = 6;
static const int RookMobOpening = 3;
static const int RookMobEndgame = 5;
static const int QueenMobOpening = 1;
static const int QueenMobEndgame = 2;
static const int KingMobOpening = 1;
static const int KingMobEndgame = 1;

static const bool UseOpenFile = true;
static const int RookSemiOpenFileOpening = 11;
static const int RookSemiOpenFileEndgame = 13;
static const int RookOpenFileOpening = 26;
static const int RookOpenFileEndgame = 27;
static const int RookSemiKingFileOpening = 11;
static const int RookKingFileOpening = 20;

static const bool UseKingAttack = true;
static const int KingAttackOpening = 28; // was 20
static const int KingAttackOpening1 = 28; // was 20

static const bool UseShelter = true;
static const int ShelterOpening = 221;
static const bool UseStorm = true;
static const int StormOpening = 9;

static const int Rook7thOpening = 19;
static const int Rook7thEndgame = 41;
static const int Queen7thOpening = 10;
static const int Queen7thEndgame = 22;

static const int TrappedBishop = 101;
...
static const int PassedOpeningMin = 14;
static const int PassedOpeningMax = 28;
static const int PassedEndgameMin = 29;
static const int PassedEndgameMax = 98;

static const int UnstoppablePasser = 563;
static const int FreePasser = 236;

static const int AttackerDistance = 7;
static const int DefenderDistance = 23;
...
/*static const int SSpaceWeight[16] = {
   0, 0, 10, 40, 70, 90, 90, 90, 90, 90, 90, 90 ,90, 90, 90, 90,
};*/
...
void eval_parameter() {
   ...
   KingSafety = option_get_bool("Toga King Safety");
...
/*   if (egbb_is_loaded && board->piece_nb <=5 && ABS(SearchBest[0].value) >= WIN_SCORE) {
   ...
   // eval_pattern(board,&opening,&endgame);
      ...
      /*if (board->piece_nb == 4){
	   if (board->number[WhiteKnight12] + board->number[WhiteBishop12] == 2){
		   eval = 0;
		   wb = board->piece[White][1];
		   if (SQUARE_COLOUR(wb) == SQUARE_COLOUR(A8)){
			   wb = MIN(DISTANCE(A8,KING_POS(board,Black)),DISTANCE(H1,KING_POS(board,Black)));
			   eval += 140 - wb * 20;
			   wb = DISTANCE(KING_POS(board,White),KING_POS(board,Black));
			   eval += 360 - wb * 40;
		   }
		   else{
			   wb = MIN(DISTANCE(A1,KING_POS(board,Black)),DISTANCE(H8,KING_POS(board,Black)));
			   eval += 140 - wb * 20;
			   wb = DISTANCE(KING_POS(board,White),KING_POS(board,Black));
			   eval += 360 - wb * 40;
		   }
	   }
	   else if (board->number[BlackKnight12] + board->number[BlackBishop12] == 2){
		   eval = 0;
		   wb = board->piece[Black][1];
		   if (SQUARE_COLOUR(wb) == SQUARE_COLOUR(A8)){
			   wb = MIN(DISTANCE(A8,KING_POS(board,White)),DISTANCE(H1,KING_POS(board,White)));
			   eval -= 140 - wb * 20;
			   wb = DISTANCE(KING_POS(board,White),KING_POS(board,Black));
			   eval -= 360 - wb * 40;
		   }
		   else{
			   wb = MIN(DISTANCE(A1,KING_POS(board,White)),DISTANCE(H8,KING_POS(board,White)));
			   eval -= 140 - wb * 20;
			   wb = DISTANCE(KING_POS(board,White),KING_POS(board,Black));
			   eval -= 360 - wb * 40;
		   }
   ...
   for (colour = 0; colour < ColourNb; colour++) {
      ...
      for (ptr = &board->piece[me][1]; (from=*ptr) != SquareNone; ptr++) { // HACK: no king
         ...
         case Bishop64:
			...
			if (me == White){
				if (board->square[from-17] == WP)
					mob += KnightOutpostMatrix[me][SquareTo64[from]]/2; 
				if (board->square[from-15] == WP)
					mob += KnightOutpostMatrix[me][SquareTo64[from]]/2; 
			}
			else{
         ...
         case Queen64:
			...
			op[me] += /* (mob * QueenMobOpening) + */ new_mob;
            eg[me] += /* (mob * QueenMobEndgame) + */ new_mob; 
...
static void eval_king(const board_t * board, const material_info_t * mat_info, int * opening, int * endgame) {
   ...
   if (UseShelter && (mat_info->cflags[White] & MatKingFlag) != 0) {
	  ...
	  if (king_is_safe[White] == false && board->number[BlackQueen12] > 0){ 
   ...
   if (UseShelter && (mat_info->cflags[Black] & MatKingFlag) != 0) {
	  ...
	  if (king_is_safe[Black] == false && board->number[WhiteQueen12] > 0){ 
