...
static const int RazorMargin = 244;
...
static /* const */ int HistoryValue = 10978; // 67%
static /* const */ int HistoryBound = 6400; // * 16384 + 50) / 100 10%=1638 15%=2458 20%=3277
static /* const */ bool UseExtendedHistory = true;
static const bool HistoryReSearch = false;

// futility pruning

static /* const */ bool UseFutility = true; // false
static const int FutilityMargin = 131;
static /* const */ int FutilityMargin1 = 131;
static /* const */ int FutilityMargin2 = 180; // Chris Formula's setting; was 300

// quiescence search

static /* const */ bool UseDelta = true; // false
static /* const */ int DeltaMargin = 74;
      ...
      if (search_type == SearchShort || best_value[SearchCurrent[ThreadId]->multipv] == ValueNone) { // first move
		  value = -full_search(board,-beta,-alpha,new_depth,height+1,new_pv,NodePV,cap_extended,ThreadId);
			if (value <= alpha){ // research
				...
				search_update_root(ThreadId);
			    value = -full_search(board,-beta,ValueInf,new_depth,height+1,new_pv,NodePV,cap_extended,ThreadId);
			}
...
//   else if(node_type != NodePV && depth <= RazorDepth &&  eval(board,alpha, beta, ThreadId) < beta - RazorMargin){
...
//   }
   ...
   while ((move=sort_next(sort,ThreadId)) != MoveNone) {
	  ...
	  if (!in_check && depth <= 6 /*SearchCurrent[ThreadId]->act_iteration / 2*/ && node_type != NodePV 
		  && new_depth < depth && value < 2 * HistoryValue / (depth + depth % 2) /*2*/ 
		  && played_nb >= 1+/*sort->capture_nb*/+depth && !move_is_dangerous(move,board)){ 
	  ...
	  if (UseFutility && node_type != NodePV && depth <= 5) {
         ...
         if (!in_check && new_depth < depth && !move_is_tactical(move,board) && !move_is_dangerous(move,board)) {
            ...
            if (opt_value == +ValueInf) {
				...
				opt_value = eval(board,alpha,beta,ThreadId) + FutilityMargin;
